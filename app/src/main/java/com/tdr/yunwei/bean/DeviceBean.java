package com.tdr.yunwei.bean;

import android.os.Parcel;
import android.os.Parcelable;




/**
 * Created by Administrator on 2016/4/26.
 */
public class DeviceBean implements Parcelable {
    private String DeviceID;
    private String DeviceType;
    private String DeviceCode;
    private String SerialNumber;
    private String AreaID;
    private String XQ;
    private String GAJ;
    private String PCS;
    private String JWH;
    private String LNG;
    private String LAT;

    private String PhotoID1;
    private String PhotoID2;
    private String PhotoID3;

    private String Photo1;
    private String Photo2;
    private String Photo3;
    private String Address;
    private String Usage;
    private String Owner;
    private String RepairCompany;
    private String CarrierOperator;
    private String IP;
    private String Mask;
    private String Gateway;
    private String Phone;
    private String SIM;
    private String AccessType;
    private String Description;
    private String Reserve1;
    private String Reserve2;
    private String Reserve3;
    private String Reserve4;
    private String Reserve5;
    private String Reserve6;
    private String Reserve7;
    private String Reserve8;
    private String Reserve9;
    private String Reserve10;
    private String Reserve11;
    private String Reserve12;
    private String Reserve13;
    private String Reserve14;
    private String Reserve15;
    private String Reserve16;
    private String Reserve17;
    private String Reserve18;
    private String Reserve19;
    private String Reserve20;
    private String Reserve21;
    private String Reserve22;
    private String Reserve23;
    private String Reserve24;
    private String Reserve25;
    private String Reserve26;
    private String Reserve27;
    private String Reserve28;
    private String Reserve29;
    private String Reserve30;

    public String getDeviceID() {
        return DeviceID;
    }

    public void setDeviceID(String deviceID) {
        DeviceID = deviceID;
    }

    public String getDeviceType() {
        return DeviceType;
    }

    public void setDeviceType(String deviceType) {
        DeviceType = deviceType;
    }

    public String getDeviceCode() {
        return DeviceCode;
    }

    public void setDeviceCode(String deviceCode) {
        DeviceCode = deviceCode;
    }

    public String getSerialNumber() {
        return SerialNumber;
    }

    public void setSerialNumber(String serialNumber) {
        SerialNumber = serialNumber;
    }

    public String getAreaID() {
        return AreaID;
    }

    public void setAreaID(String areaID) {
        AreaID = areaID;
    }

    public String getXQ() {
        return XQ;
    }

    public void setXQ(String XQ) {
        this.XQ = XQ;
    }

    public String getGAJ() {
        return GAJ;
    }

    public void setGAJ(String GAJ) {
        this.GAJ = GAJ;
    }

    public String getPCS() {
        return PCS;
    }

    public void setPCS(String PCS) {
        this.PCS = PCS;
    }

    public String getJWH() {
        return JWH;
    }

    public void setJWH(String JWH) {
        this.JWH = JWH;
    }

    public String getLNG() {
        return LNG;
    }

    public void setLNG(String LNG) {
        this.LNG = LNG;
    }

    public String getLAT() {
        return LAT;
    }

    public void setLAT(String LAT) {
        this.LAT = LAT;
    }

    public String getPhotoID1() {
        return PhotoID1;
    }

    public void setPhotoID1(String photoID1) {
        PhotoID1 = photoID1;
    }

    public String getPhotoID2() {
        return PhotoID2;
    }

    public void setPhotoID2(String photoID2) {
        PhotoID2 = photoID2;
    }

    public String getPhotoID3() {
        return PhotoID3;
    }

    public void setPhotoID3(String photoID3) {
        PhotoID3 = photoID3;
    }

    public String getPhoto1() {
        return Photo1;
    }

    public void setPhoto1(String photo1) {
        Photo1 = photo1;
    }

    public String getPhoto2() {
        return Photo2;
    }

    public void setPhoto2(String photo2) {
        Photo2 = photo2;
    }

    public String getPhoto3() {
        return Photo3;
    }

    public void setPhoto3(String photo3) {
        Photo3 = photo3;
    }

    public String getAddress() {
        return Address;
    }

    public void setAddress(String address) {
        Address = address;
    }

    public String getUsage() {
        return Usage;
    }

    public void setUsage(String usage) {
        Usage = usage;
    }

    public String getOwner() {
        return Owner;
    }

    public void setOwner(String owner) {
        Owner = owner;
    }

    public String getRepairCompany() {
        return RepairCompany;
    }

    public void setRepairCompany(String repairCompany) {
        RepairCompany = repairCompany;
    }

    public String getCarrierOperator() {
        return CarrierOperator;
    }

    public void setCarrierOperator(String carrierOperator) {
        CarrierOperator = carrierOperator;
    }

    public String getIP() {
        return IP;
    }

    public void setIP(String IP) {
        this.IP = IP;
    }

    public String getMask() {
        return Mask;
    }

    public void setMask(String mask) {
        Mask = mask;
    }

    public String getGateway() {
        return Gateway;
    }

    public void setGateway(String gateway) {
        Gateway = gateway;
    }

    public String getPhone() {
        return Phone;
    }

    public void setPhone(String phone) {
        Phone = phone;
    }

    public String getSIM() {
        return SIM;
    }

    public void setSIM(String SIM) {
        this.SIM = SIM;
    }

    public String getAccessType() {
        return AccessType;
    }

    public void setAccessType(String accessType) {
        AccessType = accessType;
    }

    public String getDescription() {
        return Description;
    }

    public void setDescription(String description) {
        Description = description;
    }

    public String getReserve1() {
        return Reserve1;
    }

    public void setReserve1(String reserve1) {
        Reserve1 = reserve1;
    }

    public String getReserve2() {
        return Reserve2;
    }

    public void setReserve2(String reserve2) {
        Reserve2 = reserve2;
    }

    public String getReserve3() {
        return Reserve3;
    }

    public void setReserve3(String reserve3) {
        Reserve3 = reserve3;
    }

    public String getReserve4() {
        return Reserve4;
    }

    public void setReserve4(String reserve4) {
        Reserve4 = reserve4;
    }

    public String getReserve5() {
        return Reserve5;
    }

    public void setReserve5(String reserve5) {
        Reserve5 = reserve5;
    }

    public String getReserve6() {
        return Reserve6;
    }

    public void setReserve6(String reserve6) {
        Reserve6 = reserve6;
    }

    public String getReserve7() {
        return Reserve7;
    }

    public void setReserve7(String reserve7) {
        Reserve7 = reserve7;
    }

    public String getReserve8() {
        return Reserve8;
    }

    public void setReserve8(String reserve8) {
        Reserve8 = reserve8;
    }

    public String getReserve9() {
        return Reserve9;
    }

    public void setReserve9(String reserve9) {
        Reserve9 = reserve9;
    }

    public String getReserve10() {
        return Reserve10;
    }

    public void setReserve10(String reserve10) {
        Reserve10 = reserve10;
    }

    public String getReserve11() {
        return Reserve11;
    }

    public void setReserve11(String reserve11) {
        Reserve11 = reserve11;
    }

    public String getReserve12() {
        return Reserve12;
    }

    public void setReserve12(String reserve12) {
        Reserve12 = reserve12;
    }

    public String getReserve13() {
        return Reserve13;
    }

    public void setReserve13(String reserve13) {
        Reserve13 = reserve13;
    }

    public String getReserve14() {
        return Reserve14;
    }

    public void setReserve14(String reserve14) {
        Reserve14 = reserve14;
    }

    public String getReserve15() {
        return Reserve15;
    }

    public void setReserve15(String reserve15) {
        Reserve15 = reserve15;
    }

    public String getReserve16() {
        return Reserve16;
    }

    public void setReserve16(String reserve16) {
        Reserve16 = reserve16;
    }

    public String getReserve17() {
        return Reserve17;
    }

    public void setReserve17(String reserve17) {
        Reserve17 = reserve17;
    }

    public String getReserve18() {
        return Reserve18;
    }

    public void setReserve18(String reserve18) {
        Reserve18 = reserve18;
    }

    public String getReserve19() {
        return Reserve19;
    }

    public void setReserve19(String reserve19) {
        Reserve19 = reserve19;
    }

    public String getReserve20() {
        return Reserve20;
    }

    public void setReserve20(String reserve20) {
        Reserve20 = reserve20;
    }

    public String getReserve21() {
        return Reserve21;
    }

    public void setReserve21(String reserve21) {
        Reserve21 = reserve21;
    }

    public String getReserve22() {
        return Reserve22;
    }

    public void setReserve22(String reserve22) {
        Reserve22 = reserve22;
    }

    public String getReserve23() {
        return Reserve23;
    }

    public void setReserve23(String reserve23) {
        Reserve23 = reserve23;
    }

    public String getReserve24() {
        return Reserve24;
    }

    public void setReserve24(String reserve24) {
        Reserve24 = reserve24;
    }

    public String getReserve25() {
        return Reserve25;
    }

    public void setReserve25(String reserve25) {
        Reserve25 = reserve25;
    }

    public String getReserve26() {
        return Reserve26;
    }

    public void setReserve26(String reserve26) {
        Reserve26 = reserve26;
    }

    public String getReserve27() {
        return Reserve27;
    }

    public void setReserve27(String reserve27) {
        Reserve27 = reserve27;
    }

    public String getReserve28() {
        return Reserve28;
    }

    public void setReserve28(String reserve28) {
        Reserve28 = reserve28;
    }

    public String getReserve29() {
        return Reserve29;
    }

    public void setReserve29(String reserve29) {
        Reserve29 = reserve29;
    }

    public String getReserve30() {
        return Reserve30;
    }

    public void setReserve30(String reserve30) {
        Reserve30 = reserve30;
    }

    public static Creator<DeviceBean> getCREATOR() {
        return CREATOR;
    }

    @Override
    public int describeContents() {
        return 0;
    }

    @Override
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeString(this.DeviceID);
        dest.writeString(this.DeviceType);
        dest.writeString(this.DeviceCode);
        dest.writeString(this.SerialNumber);
        dest.writeString(this.AreaID);
        dest.writeString(this.XQ);
        dest.writeString(this.GAJ);
        dest.writeString(this.PCS);
        dest.writeString(this.JWH);
        dest.writeString(this.LNG);
        dest.writeString(this.LAT);
        dest.writeString(this.PhotoID1);
        dest.writeString(this.PhotoID2);
        dest.writeString(this.PhotoID3);
        dest.writeString(this.Photo1);
        dest.writeString(this.Photo2);
        dest.writeString(this.Photo3);
        dest.writeString(this.Address);
        dest.writeString(this.Usage);
        dest.writeString(this.Owner);
        dest.writeString(this.RepairCompany);
        dest.writeString(this.CarrierOperator);
        dest.writeString(this.IP);
        dest.writeString(this.Mask);
        dest.writeString(this.Gateway);
        dest.writeString(this.Phone);
        dest.writeString(this.SIM);
        dest.writeString(this.AccessType);
        dest.writeString(this.Description);
        dest.writeString(this.Reserve1);
        dest.writeString(this.Reserve2);
        dest.writeString(this.Reserve3);
        dest.writeString(this.Reserve4);
        dest.writeString(this.Reserve5);
        dest.writeString(this.Reserve6);
        dest.writeString(this.Reserve7);
        dest.writeString(this.Reserve8);
        dest.writeString(this.Reserve9);
        dest.writeString(this.Reserve10);
        dest.writeString(this.Reserve11);
        dest.writeString(this.Reserve12);
        dest.writeString(this.Reserve13);
        dest.writeString(this.Reserve14);
        dest.writeString(this.Reserve15);
        dest.writeString(this.Reserve16);
        dest.writeString(this.Reserve17);
        dest.writeString(this.Reserve18);
        dest.writeString(this.Reserve19);
        dest.writeString(this.Reserve20);
        dest.writeString(this.Reserve21);
        dest.writeString(this.Reserve22);
        dest.writeString(this.Reserve23);
        dest.writeString(this.Reserve24);
        dest.writeString(this.Reserve25);
        dest.writeString(this.Reserve26);
        dest.writeString(this.Reserve27);
        dest.writeString(this.Reserve28);
        dest.writeString(this.Reserve29);
        dest.writeString(this.Reserve30);
    }

    public DeviceBean() {
    }

    protected DeviceBean(Parcel in) {
        this.DeviceID = in.readString();
        this.DeviceType = in.readString();
        this.DeviceCode = in.readString();
        this.SerialNumber = in.readString();
        this.AreaID = in.readString();
        this.XQ = in.readString();
        this.GAJ = in.readString();
        this.PCS = in.readString();
        this.JWH = in.readString();
        this.LNG = in.readString();
        this.LAT = in.readString();
        this.PhotoID1 = in.readString();
        this.PhotoID2 = in.readString();
        this.PhotoID3 = in.readString();
        this.Photo1 = in.readString();
        this.Photo2 = in.readString();
        this.Photo3 = in.readString();
        this.Address = in.readString();
        this.Usage = in.readString();
        this.Owner = in.readString();
        this.RepairCompany = in.readString();
        this.CarrierOperator = in.readString();
        this.IP = in.readString();
        this.Mask = in.readString();
        this.Gateway = in.readString();
        this.Phone = in.readString();
        this.SIM = in.readString();
        this.AccessType = in.readString();
        this.Description = in.readString();
        this.Reserve1 = in.readString();
        this.Reserve2 = in.readString();
        this.Reserve3 = in.readString();
        this.Reserve4 = in.readString();
        this.Reserve5 = in.readString();
        this.Reserve6 = in.readString();
        this.Reserve7 = in.readString();
        this.Reserve8 = in.readString();
        this.Reserve9 = in.readString();
        this.Reserve10 = in.readString();
        this.Reserve11 = in.readString();
        this.Reserve12 = in.readString();
        this.Reserve13 = in.readString();
        this.Reserve14 = in.readString();
        this.Reserve15 = in.readString();
        this.Reserve16 = in.readString();
        this.Reserve17 = in.readString();
        this.Reserve18 = in.readString();
        this.Reserve19 = in.readString();
        this.Reserve20 = in.readString();
        this.Reserve21 = in.readString();
        this.Reserve22 = in.readString();
        this.Reserve23 = in.readString();
        this.Reserve24 = in.readString();
        this.Reserve25 = in.readString();
        this.Reserve26 = in.readString();
        this.Reserve27 = in.readString();
        this.Reserve28 = in.readString();
        this.Reserve29 = in.readString();
        this.Reserve30 = in.readString();
    }

    public static final Creator<DeviceBean> CREATOR = new Creator<DeviceBean>() {
        @Override
        public DeviceBean createFromParcel(Parcel source) {
            return new DeviceBean(source);
        }

        @Override
        public DeviceBean[] newArray(int size) {
            return new DeviceBean[size];
        }
    };
}
